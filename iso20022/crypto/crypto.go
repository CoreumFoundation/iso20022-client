...
func GenerateSharedKey(ctx coreumchainclient.Context, localAccount, remoteAccount sdk.AccAddress) ([]byte, error) {
	// getting private key
...
func generateSecp256k1SharedKey(privateKey *secp256k1.PrivKey, publicKey *secp256k1.PubKey) ([]byte, error) {
	// preparing private key
	pvKey := secp256k1v4.PrivKeyFromBytes(privateKey.Bytes())

	// preparing public key
	pbKey, err := secp256k1v4.ParsePubKey(publicKey.Bytes())
	if err != nil {
		return nil, err
	}
...
func generateSecp256r1SharedKey(privateKey *secp256r1.PrivKey, publicKey *secp256r1.PubKey) ([]byte, error) {
	// preparing private key
	...
	// preparing public key
	ecdhPub, err := publicKey.Key.PublicKey.ECDH()
	if err != nil {
		return nil, err
	}
